...
function fakeEmbedding(s: string): Buffer {
  ...
  for (let i = 0; i < arr.length; i++) arr[i] = ((h ^ (i * 2654435761)) % 1000) / 1000;
  return Buffer.from(arr.buffer);
}

const dir = process.argv[2] || path.join(process.cwd(), 'seed', 'sources');
ingestDir(dir).then(() => { console.log('Ingestion done'); process.exit(0); }).catch((e) => { console.error(e); process.exit(1); });
